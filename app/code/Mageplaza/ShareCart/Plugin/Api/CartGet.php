...
class CartGet
{
    ...
    public function setTokenToExtensionAttributes(CartInterface $cart)
    {
        ...
        // Check if the getMpShareCartToken method exists
        if ($cartExtension && method_exists($cartExtension, 'getMpShareCartToken')) {
            if ($cartExtension->getMpShareCartToken()) {
                return $cart;
            }
        }
        ...
        // Ensure setMpShareCartToken exists before calling it
        if (method_exists($cartExtension, 'setMpShareCartToken')) {
